...
to go
  ...
  layout-circle bugs world-radius
  ask bugs
  [ set $fitness 0
...
to bug.run
  ...
  [ if show-info? [ print (word self " => reward") ]
    set $fitness ($fitness + 1)
    ;; relocate
    move-to one-of patches with [(square pxcor) + (square pycor) = (square world-radius)]
  ]
end
...
to-report sensor.obj-fwd [#obj-ebreed]
  ;print "sensor.fwd"
  report any? (#obj-ebreed in-cone *sight-distance* 90)
end

to-report sensor.obj-side [#obj-ebreed #angle]
  ;print (word "sense " #angle)
  right #angle
